...
function startName(time){
  ...
  if($("#start_name").html()==="开始点名"){
      ...
      timeId=setInterval(function () {
          ...
          //留下当前层的颜色
          $("#student_name ul").children()[aa].style.background='#AF85FF url(program/img/icon.png) no-repeat center 3px';

          //语音播报
          var ttsDiv = document.getElementById('bdtts_div_id');
          var ttsAudio = document.getElementById('tts_autio_id');
          var ttsText = $("#student_name ul").children()[aa].innerHTML;
...
function answerName(){
  if($("#answer_name").html()==="点人答题"){
      ...
      timeIds=setInterval(function () {
        ...
        $("#student_name ul").children()[rand].style.background='red url(program/img/icon.png) no-repeat center 3px';
      },300);
...
function time_unavailable(){
  ...
  $('#start_name')[0].disabled=true;
  $('#import_name')[0].disabled=true;
  $('#out_name')[0].disabled=true;
  $('#answer_name')[0].disabled=true;
  $('#start_name').addClass('time_unavailable');
  $('#import_name').addClass('time_unavailable');
  $('#out_name').addClass('time_unavailable');
  $('#answer_name').addClass('time_unavailable');
  $('.btns_box button').addClass('time_unavailable');
...
function time_available(){
  ...
  $('#import_name')[0].disabled=false;
  $('#out_name')[0].disabled=false;
  $('#answer_name')[0].disabled=false;
  $('#start_name')[0].disabled=false;
  $('#start_name').removeClass('time_unavailable');
  $('#import_name').removeClass('time_unavailable');
  $('#out_name').removeClass('time_unavailable');
  $('#answer_name').removeClass('time_unavailable');
  $('.btns_box button').removeClass('time_unavailable');
...
function getTime(){
    ...
    $("#timers").text(year+"-"+month+"-"+dat+" "+hour+":"+minitue+":"+second)
}
